...
- **2025年7月19日**: **v2.0 モジュール構造改革版** - 70%コード削減、A級性能評価、Phase 1-4モジュール統合完了
...
### 実環境テスト優先の絶対原則
1. **実ハーメルンページでの動作確認を最優先**
   - 軽量な実ページでの定期的動作確認
   - 実際のCSS、画像、JSリソースでの処理検証
   - Cloudflare認証通過の確認
...
3. **段階的品質確認プロセス**
   - レベル1：基本ロジックテスト（モック可）
   - レベル2：実ハーメルンページテスト（必須）
   - レベル3：エンドユーザー使用シーン検証（必須）

## ⚠️ 重大ミス記録：AIモックテスト偏重事件（2025-07-20）
...
### 予防策完全版
1. **実ハーメルンテスト必須化** - 全修正作業で実施
2. **AIモック限界明記** - 設計文書に常時記載
3. **ユーザー提案最優先** - AI判断より常に優先
4. **HTML構造の定期再調査** - 月次での実構造確認
5. **段階的品質管理** - 基本→実環境→ユーザー体験の3段階
- 定期的な本質目的の確認
- AIモックの限界を常に認識

### 学んだ教訓
**「テスト100%成功」の真の意味は数字ではなく、実際のハーメルンで確実に動作することである**

## 🚨 ユーザー提案軽視防止の絶対原則

### Claude行動の強制ルール
1. **ユーザー提案 > AI判断の絶対優先**
   - ユーザーが「実際のサイトでテスト」と提案 → 必ず実施
   - AI判断で「モックで十分」→ 絶対禁止
   - ユーザー要求の本質理解を最優先

2. **AI限界の自己認識強制**
   - AIは実ハーメルン構造を完全再現できない
   - 「想像した構造」は常に不完全
   - 実環境確認なしでの「十分」判断を禁止

3. **同一ミス反復の絶対防止**
   - 過去に記録したミスを再度犯すことを厳格禁止
   - ユーザー指摘 → 即座に認識・修正
   - 表面的対応での満足を完全排除
...
## ⚠️ 重大ミス記録3：ユーザー提案軽視事件継続（2025-07-20続）
...
### 学んだ教訓（更新）
**ユーザー提案こそが正解。AIの判断は常に不完全。実環境確認こそが唯一の品質保証。**